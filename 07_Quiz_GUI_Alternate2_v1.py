...
This is the first version of a second alternate method for this component (Component 7). This version displays a
correct message in a 'light green' font and an incorrect message in a 'pink' font. The entry box changes colour as well
...
class Quiz:
    ...
    def mark_answer(self):
        ...
        if capital == inputted_capital:
            ...
            self.entry_box.config(bg=correct)
            ...
            self.entry_box.config(bg=incorrect)
            self.next_button.config(state=NORMAL)  # Reactivating the next button
            if len(capitals) > 0:
                capitals.pop(get_country)
            if len(capitals) > 0:
