...
This version displays the statistics of the quiz.
Version 6 – 10/09/2021
"""
...
class Quiz:
    def __init__(self):
        ...
        result_statistics = "Correct: {}\n" \
                            "Total Answered: {}"

        # Label where the statistics are displayed
        self.statistics_label = Label(self.capital_quiz_frame, text=result_statistics, bg=background_colour, fg="black",
                                      font="Arial 12")
        self.statistics_label.grid(row=8)
    ...
    def mark_answer(self):
        global get_country, country, capital, correctly_answered, total_answered
        correct = "green"
        incorrect = "red"
        error = "no"
        valid = "[A-Za-z0-9_]"
        ...
        for character in inputted_capital:
            if re.match(valid, character):
                continue
            elif character == " ":  # When the filename contains a space
                error_message = "The input cannot contain a space"
            else:
                error_message = "The input cannot contain {}".format(character)
            error = "yes"
            break

        if error == "yes":
            # Displaying an error message
            self.result_label.config(text="Invalid Input – {}\n".format(error_message), fg=incorrect)
        else:
            if capital == inputted_capital:
                self.result_label.configure(text="Correct", fg=correct)
                correctly_answered += 1
                total_answered += 1
                self.statistics_label.config(text="Correct: {}\n"
                                                  "Total Answered: {}".format(correctly_answered, total_answered))
                ...
                if len(capitals) > 0:
                    get_country = random.randint(0, len(capitals) - 1)  # This gets a list from within the whole list
                    country = capitals[get_country][1]  # This gets the country from the list
                    # This gets the respective capital for the country in the line above
                    capital = capitals[get_country][0]
            else:
                self.result_label.configure(text="Incorrect. The correct answer was {}".format(capital), fg=incorrect)
                total_answered += 1
                self.statistics_label.config(text="Correct: {}\n"
                                                  "Total Answered: {}".format(correctly_answered, total_answered))
...
class History:
    def __init__(self, partner, quiz_history):
        ...
        self.history_results_label = Label(self.history_frame, text=history_string, bg=background, font="Arial 12",
                                           justify=LEFT)
        self.history_results_label.grid(row=2)
...
class Export:
    ...
    def save_history(self, partner, quiz_history):
        ...
        if error == "yes":
            ...
            file.write("Total Answered: {}".format(len(quiz_history)))
            file.close()
